...
const fetchDataById = async (id) => {
  try {
    ...
    if (!response.ok) { // if the response status is not ok (e.g., 404 Not Found)
...
const closeModalWithEdit = async () => {
  ...
  try {
    const response = await fetch(`http://localhost:8080/itb-kk/v1/tasks/${selectedTodo.value.id}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify(selectedTodo.value),
    });
    if (!response.ok) {
      throw new Error(`HTTP error! status: ${response.status}`);
    }
    const data = await response.json();
    // Update the task in allTodos
    const index = allTodos.value.findIndex(todo => todo.id === data.id);
    if (index !== -1) {
      allTodos.value[index] = data;
    }
    errorMessage.value = '';
...
<template>
    ...
    <div v-if="showEditModal" class="fixed z-10 inset-0 overflow-y-auto flex items-center justify-center">
        <div class="bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:max-w-2xl sm:w-full bg-opacity-50">
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div class="sm:flex sm:items-start">
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        <h3 class="text-lg leading-6 font-medium text-gray-900">
                            <input type="text" v-model="selectedTodo.title" class="block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                        </h3>
                        <div class="mt-2">
                            <textarea v-model="selectedTodo.description" class="block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md h-32">
                                {{ selectedTodo.description || 'No Description Provided' }}
                            </textarea>
                        </div>
                        <div class="mt-2">
                            <label for="status" class="block text-sm font-medium text-gray-700">Status</label>
                            <select id="status" v-model="selectedTodo.status" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                        ...
                        <div class="mt-2">
                            <label class="block text-sm font-medium text-gray-700">Updated On</label>
            ...
            <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                <button @click="closeModalWithEdit"
